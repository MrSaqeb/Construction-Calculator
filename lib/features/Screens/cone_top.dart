...
class ConeTop extends ConsumerStatefulWidget {
  const ConeTop({super.key});

  @override
  ConsumerState<ConeTop> createState() => _ConeTopState();
}

class _ConeTopState extends ConsumerState<ConeTop> {
  ...
  void _calculate() {
    final double? topDiameter = double.tryParse(topdiameterController.text);
    final double? bottomDiameter = double.tryParse(
      bottomdiameterController.text,
    );
    final double? cylinderHeight = double.tryParse(
      cylinderheightController.text,
    );
    final double? coneHeight = double.tryParse(coneheightController.text);
    final double? filledHeight = double.tryParse(filledController.text);
    ...
    final double topRadius =
        (topDiameter / 2) * (unitToMeter[selectedUnit] ?? 1.0);
    final double bottomRadius =
        (bottomDiameter / 2) * (unitToMeter[selectedUnit] ?? 1.0);
    final double cylinderH =
        cylinderHeight * (unitToMeter[selectedUnit] ?? 1.0);
    final double coneH = coneHeight * (unitToMeter[selectedUnit] ?? 1.0);
    double filledH = (filledHeight ?? 0) * (unitToMeter[selectedUnit] ?? 1.0);

    // Volume of cylinder part (using bottom radius)
    final double cylinderVolume = pi * pow(bottomRadius, 2) * cylinderH;

    // Volume of top cone (frustum if top radius different from bottom of cone)
    final double coneVolume =
        (1 / 3) *
        pi *
        coneH *
        (pow(bottomRadius, 2) + bottomRadius * topRadius + pow(topRadius, 2));

    final double totalM3 = cylinderVolume + coneVolume;

    // Calculate filled volume
    double filledM3;
    if (filledH <= cylinderH) {
      filledM3 = pi * pow(bottomRadius, 2) * filledH;
    } else {
      double coneFilledH = filledH - cylinderH;
      if (coneFilledH > coneH) coneFilledH = coneH;
      // Frustum filled proportionally
      double frustumFilledVolume =
          (1 / 3) *
          pi *
          coneFilledH *
          (pow(bottomRadius, 2) +
              bottomRadius * topRadius * (coneFilledH / coneH) +
              pow(topRadius * (coneFilledH / coneH), 2));
      filledM3 = cylinderVolume + frustumFilledVolume;
    }
  ...
  void _saveHistory() {
    ...
    ref.read(unifiedHistoryProvider.notifier).addConeHistory(item);
  }
          ...
          children: [
            Center(
              child: SvgPicture.asset(
                'assets/icons/cone_top_icon.svg',
                height: 100,
                color: orangeColor,
              ),
            ),
              ...
              children: [
                Text(
                  ...
                  style: TextStyle(
                    ...
                    fontSize: 16,
                    fontWeight: FontWeight.w600,
                    color: Colors.black, // ya theme color
                  ),
                ),
                Column(
                  ...
                  children: const [
                    Text(
                      "Cylinder Volume: V_cyl = π × R_bottom² × H_cyl",
                      style: TextStyle(
                        fontFamily: 'Poppins',
                        fontSize: 9,
                        fontWeight: FontWeight.w600,
                    ...
                    Text(
                      "Cone Volume: V_cone = (1/3) × π × H_cone × \n(R_bottom² + R_bottom×R_top + R_top²)",
                      style: TextStyle(
                        fontFamily: 'Poppins',
                        fontSize: 8,
                        fontWeight: FontWeight.w600,
                    ...
                    Text(
                      "Total Volume: V_total = V_cyl + V_cone",
                      style: TextStyle(
                        fontFamily: 'Poppins',
                        fontSize: 8,
                        fontWeight: FontWeight.w600,
