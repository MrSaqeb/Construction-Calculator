...
class MassUnitConverter extends ConsumerStatefulWidget {
  const MassUnitConverter({super.key});

  @override
  ConsumerState<MassUnitConverter> createState() => _MassUnitConverterState();
}

class _MassUnitConverterState extends ConsumerState<MassUnitConverter> {
  ...
  // Conversion logic
  void _calculate() {
    final double? inputVal = double.tryParse(massController.text);
    if (inputVal == null) return;

    // Conversion factors (base = kilogram)
    final Map<String, double> massToKg = {
      'Kilogram': 1,
      'Gram': 0.001,
      'Milligram': 0.000001,
      'Microgram': 0.000000001,
      'Tonne': 1000,
      'US Ton': 907.1847, // short ton
      'Stone': 6.35029,
      'Pound': 0.453592,
      'Ounce': 0.0283495,
    };

    final double baseKg = inputVal * massToKg[selectedUnit]!;

    setState(() {
      convertedValues = {};
      massToKg.forEach((unit, factor) {
        final double value = baseKg / factor;
  ...
  void _saveHistory() {
    ...
    ref.read(unifiedHistoryProvider.notifier).addMass(item);
  }
