...
class WeightUnitConverter extends ConsumerStatefulWidget {
  const WeightUnitConverter({super.key});

  @override
  ConsumerState<WeightUnitConverter> createState() =>
      _WeightUnitConverterState();
}

class _WeightUnitConverterState extends ConsumerState<WeightUnitConverter> {
  ...
  void _calculate() {
    final double? inputVal = double.tryParse(weightController.text);
    if (inputVal == null) return;

    // conversion factors (base = kilogram)
    final Map<String, double> weightUnitToKg = {
      'Kilogram': 1.0,
      'Gram': 0.001,
      'Milligram': 0.000001,
      'Metric Ton': 1000.0,
    ...
    final double baseKg = inputVal * weightUnitToKg[selectedUnit]!;

    setState(() {
      convertedValues = {};
      weightUnitToKg.forEach((unit, factor) {
