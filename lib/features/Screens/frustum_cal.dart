...
class FrustumCal extends ConsumerStatefulWidget {
  const FrustumCal({super.key});

  @override
  ConsumerState<FrustumCal> createState() => _FrustumCalState();
}

class _FrustumCalState extends ConsumerState<FrustumCal> {
  ...
  void _calculate() {
    ...
    if (topDiameter == null || bottomDiameter == null || coneHeight == null) {
      setState(() {
        totalVolume = 0;
        filledVolume = 0;
        convertedValues = {};
    ...
    final double h = (coneHeight) * (unitToMeter[selectedUnit] ?? 1.0);

    // Clamp filled height if necessary
    double hFilled = (filledHeight ?? 0) * (unitToMeter[selectedUnit] ?? 1.0);
    if (hFilled > h) hFilled = h;

    // Frustum volume formula: V = (1/3) * π * H * (R1² + R1*R2 + R2²)
    ...
    // Filled volume proportional to height (simple linear assumption)
    final double filledM3 = (hFilled / h) * totalM3;
  ...
  void _saveHistory() {
    ...
    ref.read(unifiedHistoryProvider.notifier).addFrustum(item);
  }
            ...
            Row(
              ...
              children: [
                ...
                Column(
                  ...
                  children: const [
                    Text(
                      "V_total = (1/3) × π × H × (R_bottom²\n + R_bottom×R_top + R_top²)",
