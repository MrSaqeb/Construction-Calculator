...
class TimeUnitConverter extends ConsumerStatefulWidget {
  const TimeUnitConverter({super.key});

  @override
  ConsumerState<TimeUnitConverter> createState() => _TimeUnitConverterState();
}

class _TimeUnitConverterState extends ConsumerState<TimeUnitConverter> {
  final Color orangeColor = const Color(0xFFFF9C00);
  final TextEditingController timeController = TextEditingController();

  // Selected unit
  String selectedUnit = "Second";
  ...
  // Conversion logic
  void _calculate() {
    final double? inputVal = double.tryParse(timeController.text);
    if (inputVal == null) return;

    // Conversion factors (base = second)
    final Map<String, double> timeToSecond = {
      ...
      'Day': 86400,
      'Week': 604800,
      'Month': 2629800, // average 30.44 days
      'Year': 31557600, // 365.25 days
    };

    final double baseSecond = inputVal * timeToSecond[selectedUnit]!;

    setState(() {
      convertedValues = {};
      timeToSecond.forEach((unit, factor) {
        final double value = baseSecond / factor;
  ...
  void _saveHistory() {
    ...
    ref.read(unifiedHistoryProvider.notifier).addTime(item);
  }
