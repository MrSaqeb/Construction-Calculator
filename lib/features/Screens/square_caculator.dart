...
class SquareConverter extends ConsumerStatefulWidget {
  const SquareConverter({super.key});

  @override
  ConsumerState<SquareConverter> createState() => _SquareConverterState();
}

class _SquareConverterState extends ConsumerState<SquareConverter> {
  ...
  // Conversion logic
  void _calculate() {
    final double? inputVal = double.tryParse(lenghtController.text);
    ...
    // ✅ Step 1: Length in meters
    final double lengthInMeter = inputVal * unitToMeter[selectedUnit]!;

    // ✅ Step 2: Area (m²) and Perimeter (m)
    final double areaM2 = lengthInMeter * lengthInMeter;

    // ✅ Step 3: Convert back to selected unit
    final double lengthInSelected = lengthInMeter / unitToMeter[selectedUnit]!;
    final double areaInSelected = lengthInSelected * lengthInSelected;
    final double perimeterInSelected = 4 * lengthInSelected;

    setState(() {
      areaResult = areaInSelected;
      perimeterResult = perimeterInSelected;

      // ✅ Conversion table (Area conversions)
      convertedValues = {
        'meter²': areaM2,
        'feet²': areaM2 * 10.7639,
        'yard²': areaM2 * 1.19599,
          ...
          children: [
            ...
            _suffixInputWithDropdown(
              ...
              onChanged: (v) {
                ...
                _calculate(); // ✅ dropdown change hone par auto calculate
              },
              onChangedInput: (value) {
                _calculate(); // ✅ input change hone par auto calculate
              },
            ),
