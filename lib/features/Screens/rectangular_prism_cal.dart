...
class RectangularPrismCal extends ConsumerStatefulWidget {
  const RectangularPrismCal({super.key});

  @override
  ConsumerState<RectangularPrismCal> createState() =>
      _RectangularPrismCalState();
}

class _RectangularPrismCalState extends ConsumerState<RectangularPrismCal> {
  ...
  void _calculate() {
    final double? lengthInput = double.tryParse(base1Controller.text); // Length
    final double? widthInput = double.tryParse(base2Controller.text); // Width
    final double? heightInput = double.tryParse(
      heightController.text,
    ); // Total height
    final double? filledInput = double.tryParse(
    ...
    final double totalM3 = lengthM * widthM * heightM;
    final double filledM3 = lengthM * widthM * filledM;
  ...
  void _saveHistory() {
    ...
    final item = RectangularPrismItem(
      length: lengthVal,
      width: widthVal,
      height: heightVal,
    ...
    ref.read(unifiedHistoryProvider.notifier).addRectangularPrism(item);
  }
          ...
          children: [
            Center(
              child: SvgPicture.asset(
                'assets/icons/tankrectangle.svg',
                height: 100,
                color: orangeColor,
              ),
            ),
