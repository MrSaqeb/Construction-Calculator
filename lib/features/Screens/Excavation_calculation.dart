...
class ExcavationCalculationScreen extends ConsumerStatefulWidget {
  const ExcavationCalculationScreen({super.key});

  @override
  ConsumerState<ExcavationCalculationScreen> createState() =>
      _ExcavationCalculationScreenState();
}

class _ExcavationCalculationScreenState
    extends ConsumerState<ExcavationCalculationScreen> {
  // THEME ACCENTS
  ...
  void _calculate() {
    // 1️⃣ Convert all inputs to meters
    final length = _toMetersLength();
    final width = _toMetersWidth();
    final depth = _toMetersDepth();

    // 2️⃣ Calculate volume (m³)
    final totalVolume = length * width * depth;

    setState(() {
      volume = totalVolume;
    });

    // 3️⃣ Save to Hive & UnifiedHistory
    saveExcavationHistory(length, width, depth, totalVolume);
  }

  Future<void> saveExcavationHistory(
    double length,
    double width,
    double depth,
    double volume,
  ) async {
    // 1️⃣ Create new history item
    final newHistoryItem = ExcavationHistoryItem(
      length: length,
      width: width,
      depth: depth,
      volume: volume,
    ...
    ref
        ...
        .addExcavationHistory(newHistoryItem);

    // 4️⃣ SnackBar confirmation
    // ScaffoldMessenger.of(context).showSnackBar(
    //   const SnackBar(content: Text('Excavation history saved successfully!')),
    // );
  }
          ...
          children: [
            Center(
              child: SvgPicture.asset(
                'assets/icons/excavation_icon.svg',
                height: 85,
                color: orangeColor,
              ),
            ),
