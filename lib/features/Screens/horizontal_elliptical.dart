...
class HorizontalElliptical extends ConsumerStatefulWidget {
  const HorizontalElliptical({super.key});

  @override
  ConsumerState<HorizontalElliptical> createState() =>
      _HorizontalEllipticalState();
}

class _HorizontalEllipticalState extends ConsumerState<HorizontalElliptical> {
  ...
  void _calculate() {
    final double? lengthInput = double.tryParse(
    ...
    // Total volume for horizontal elliptical tank
    final double totalM3 = (pi / 4) * lengthM * widthM * heightM;

    // Filled volume proportional to height
  ...
  void _saveHistory() {
    ...
    final item = HorizontalEllipticalHistoryItem(
      length: lengthVal,
      width: widthVal,
      height: heightVal,
      filled: filledVal,
    ...
    ref.read(unifiedHistoryProvider.notifier).addHorizontalElliptical(item);
  }
          ...
          children: [
            Center(
              child: SvgPicture.asset(
                'assets/icons/horizontal_elliptical_icon.svg',
                height: 100,
                color: orangeColor,
              ),
            ),
