...
class SqaureBarCalculator extends ConsumerStatefulWidget {
  const SqaureBarCalculator({super.key});

  @override
  ConsumerState<SqaureBarCalculator> createState() =>
      _SqaureBarCalculatorState();
}

class _SqaureBarCalculatorState extends ConsumerState<SqaureBarCalculator> {
  // THEME ACCENTS
  ...
  final TextEditingController widthWController = TextEditingController();
  ...
  void _calculate() {
    final double side =
        double.tryParse(heightController.text) ?? 0.0; // Square side
    final double length = double.tryParse(lengthController.text) ?? 0.0;
    final int pieces = int.tryParse(piecesController.text) ?? 1;
    final double costPerUnit =
        double.tryParse(costOfSteelController.text) ?? 0.0;

    final double sideM = convertToMeter(side, selectedLengthUnit);
    ...
    final double volume = sideM * sideM * lengthM * pieces;

    // Base weight in kg
    weightkg = volume * density;

    // Convert weight for display according to selected unit
    double displayedWeight = convertWeight(weightkg!, selectedWeightUnit);

    // Update weighttons only if selected unit is ton
    weighttons = selectedWeightUnit.toLowerCase() == "ton"
        ? displayedWeight
        : weightkg! / 1000;

    // Cost always calculated per kg
    cost = weightkg! * costPerUnit;

    setState(() {});
    _saveSquareBarHistory();
  }

  void _saveSquareBarHistory() {
    if (weightkg == null || weighttons == null || cost == null) return;

    // --- Parse inputs ---
    final double side = double.tryParse(heightController.text) ?? 0.0;
    final double length = double.tryParse(lengthController.text) ?? 0.0;
    final int pieces = int.tryParse(piecesController.text) ?? 1;
    final double costPerKg = double.tryParse(costOfSteelController.text) ?? 0.0;

    // --- Create history item ---
    final item = SquareBarHistoryItem(
      side: side,
      length: length,
      pieces: pieces,
    ...
    // --- Add to provider / Hive box ---
    ref.read(unifiedHistoryProvider.notifier).addSquareBar(item);

    // // --- Snackbar ---
    // ScaffoldMessenger.of(
    //   context,
    // ).showSnackBar(const SnackBar(content: Text("Square Bar history saved!")));
  }
  ...
  final List<String> weightUnitOptions = const ["kg", "Ib", "ton"];
          ...
          children: [
            Center(
              child: SvgPicture.asset(
                'assets/icons/square_bar_icon.svg',
                height: 85,
                color: orangeColor,
              ),
            ),
