...
class PressureUnitConverter extends ConsumerStatefulWidget {
  const PressureUnitConverter({super.key});

  @override
  ConsumerState<PressureUnitConverter> createState() =>
      _PressureUnitConverterState();
}

class _PressureUnitConverterState extends ConsumerState<PressureUnitConverter> {
  ...
  // Conversion logic
  void _calculate() {
    final double? inputVal = double.tryParse(pressureController.text);
    if (inputVal == null) return;

    // Conversion factors (base = Pascal)
    final Map<String, double> pressureToPascal = {
      'Pascal': 1,
      'Bar': 100000,
      'PSI': 6894.757,
      'Torr': 133.322,
    ...
    final double basePa = inputVal * pressureToPascal[selectedUnit]!;

    setState(() {
      convertedValues = {};
      pressureToPascal.forEach((unit, factor) {
        final double value = basePa / factor;
  ...
  void _saveHistory() {
    ...
    final item = PressureHistoryItem(
      inputValue: inputVal,
    ...
    ref.read(unifiedHistoryProvider.notifier).addPressure(item);
  }
