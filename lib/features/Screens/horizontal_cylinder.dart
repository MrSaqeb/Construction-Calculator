...
class HorizontalCylinder extends ConsumerStatefulWidget {
  const HorizontalCylinder({super.key});

  @override
  ConsumerState<HorizontalCylinder> createState() => _HorizontalCylinderState();
}

class _HorizontalCylinderState extends ConsumerState<HorizontalCylinder> {
  ...
  void _calculate() {
    ...
    final double? lengthInput = double.tryParse(base2Controller.text);
    final double? filledInput = double.tryParse(heightController.text);

    if (diameterInput == null || lengthInput == null) {
      setState(() {
        totalVolume = 0;
        filledVolume = 0;
        convertedValues = {};
    ...
    // Total volume remains same as vertical cylinder formula
    final double totalM3 = pi * radiusM * radiusM * lengthM;

    // Horizontal cylinder filled volume formula
    double filledHeightM =
        (filledInput ?? 0) * (unitToMeter[selectedUnit] ?? 1.0);
    if (filledHeightM > 2 * radiusM) filledHeightM = 2 * radiusM; // Clamp

    // Partial fill formula
    final double theta = 2 * acos((radiusM - filledHeightM) / radiusM);
    final double segmentArea = (radiusM * radiusM / 2) * (theta - sin(theta));
    final double filledM3 = segmentArea * lengthM;
  ...
  void _saveHistory() {
    ...
    final item = HorizontalCylinderItem(
      diameter: diameterVal,
      length: heightVal, // horizontal cylinder length instead of height
      filledHeight: filledVal,
      unit: selectedUnit,
      totalVolume: totalVolume,
      filledVolume: filledVolume,
    );
    ref.read(unifiedHistoryProvider.notifier).addHorizontalCylinder(item);
  }
