...
class CementCalculatorScreen extends ConsumerStatefulWidget {
  const CementCalculatorScreen({super.key});

  @override
  ConsumerState<CementCalculatorScreen> createState() =>
      _CementCalculatorScreenState();
}

class _CementCalculatorScreenState
    extends ConsumerState<CementCalculatorScreen> {
  // THEME ACCENTS
  ...
  void _calculate() {
    ...
    final lengthM = _toMetersLength();
    final heightM = _toMetersHeight();
    final depthM = _toMetersDepth();

    // Raw volume in cubic meters
    final volM3 = lengthM * heightM * depthM;
    ...
    // Dry volume
    final dryVol = volM3 * 1.54;

    // Cement calculation (bags)
    final cementVol = dryVol * (ratios[0] / totalParts);
    final cementBagsCalc = cementVol / 0.0347;

    // Sand calculation (CFT)
    final sandVol = dryVol * (ratios[1] / totalParts);
    final sandCftCalc = sandVol * 35.3147;

    // Aggregate calculation (CFT)
    final aggVol = dryVol * (ratios[2] / totalParts);
    final aggregateCftCalc = aggVol * 35.3147;

    setState(() {
      volume = double.parse(volM3.toStringAsPrecision(4));
      cementBags = double.parse(cementBagsCalc.toStringAsPrecision(4));
      sandCft = double.parse(sandCftCalc.toStringAsPrecision(4));
      aggregateCft = double.parse(aggregateCftCalc.toStringAsPrecision(4));
    });

    saveCementHistory(ref);
  }
  ...
  Future<void> saveCementHistory(WidgetRef ref) async {
    final historyItem = CementHistoryItem(
      lenM: lenM.text,
      lenCM: lenCM.text,
      lenFT: lenFT.text,
      lenIN: lenIN.text,
      ...
      htFT: htFT.text,
      htIN: htIN.text,
      depthCM: depthCM.text,
      depthIN: depthIN.text,
      grade: selectedGrade,
      volume: volume ?? 0.0,
            ...
            if (selectedUnit == 'Meter/CM') ...[
              ...
              Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  _suffixInput(controller: depthCM, unit: 'CM'),
                ],
              ),
            ] else ...[
              ...
              Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  _suffixInput(controller: depthIN, unit: 'Inches'),
                ],
              ),
            ],
