...
class HorizontalCapsule extends ConsumerStatefulWidget {
  const HorizontalCapsule({super.key});

  @override
  ConsumerState<HorizontalCapsule> createState() => _HorizontalCapsuleState();
}

class _HorizontalCapsuleState extends ConsumerState<HorizontalCapsule> {
  ...
  void _calculate() {
    ...
    final double? cylinderLengthInput = double.tryParse(base2Controller.text);
    final double? filledInput = double.tryParse(heightController.text);

    if (diameterInput == null || cylinderLengthInput == null) {
      setState(() {
        totalVolume = 0;
        filledVolume = 0;
        convertedValues = {};
    ...
    final double cylinderLengthM =
        cylinderLengthInput * (unitToMeter[selectedUnit] ?? 1.0);

    // Total volume = cylinder + 2 hemispheres
    final double totalM3 =
        pi * radiusM * radiusM * cylinderLengthM +
        (4.0 / 3.0) * pi * pow(radiusM, 3) * 2;

    // Filled volume
    double filledHeightM =
        (filledInput ?? 0) * (unitToMeter[selectedUnit] ?? 1.0);
    double maxLength = cylinderLengthM + 2 * radiusM;
    if (filledHeightM > maxLength) filledHeightM = maxLength;

    double filledM3 = (filledHeightM / maxLength) * totalM3;
  ...
  void _saveHistory() {
    ...
    final double? lengthVal = double.tryParse(base2Controller.text);
    final double? filledVal = double.tryParse(heightController.text);

    if (diameterVal == null || lengthVal == null || filledVal == null) return;

    final item = HorizontalCapsuleItem(
      diameter: diameterVal,
      cylinderLength: lengthVal,
      filledLength: filledVal,
      unit: selectedUnit,
    ...
    ref.read(unifiedHistoryProvider.notifier).addHorizontalCapsule(item);
  }
          ...
          children: [
            Center(
              child: SvgPicture.asset(
                'assets/icons/horizontal_capsule_icon.svg',
                height: 100,
                color: orangeColor,
              ),
            ),
