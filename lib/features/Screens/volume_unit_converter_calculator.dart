...
class VolumeUnitConverter extends ConsumerStatefulWidget {
  const VolumeUnitConverter({super.key});

  @override
  ConsumerState<VolumeUnitConverter> createState() =>
      _VolumeUnitConverterState();
}

class _VolumeUnitConverterState extends ConsumerState<VolumeUnitConverter> {
  ...
  String selectedUnit = "Cubic Meter";

  void _calculate() {
    final double? inputVal = double.tryParse(volumeController.text);
    if (inputVal == null) return;

    // Volume conversion factors (base = cubic meter)
    final Map<String, double> volumeUnitToCubicMeter = {
      'Cubic Meter': 1.0,
    ...
    final double baseCubicMeter =
        inputVal * volumeUnitToCubicMeter[selectedUnit]!;

    setState(() {
      convertedValues = {};
      volumeUnitToCubicMeter.forEach((unit, factor) {
        final double value = baseCubicMeter / factor;
  ...
  Widget _resultTable(Color textColor) {
    ...
    final rows = convertedValues.entries.map((e) {
      ...
      return [
        unit,
        value.toStringAsFixed(4), // zyada precision
      ];
    }).toList();
        ...
        child: Column(
          ...
          children: [
            ...
            _suffixInputWithDropdown(
              ...
              dropdownOptions: volumeUnitOptions,
