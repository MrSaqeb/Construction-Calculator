...
class TBarCalculator extends ConsumerStatefulWidget {
  const TBarCalculator({super.key});

  @override
  ConsumerState<TBarCalculator> createState() => _TBarCalculatorState();
}

class _TBarCalculatorState extends ConsumerState<TBarCalculator> {
  // THEME ACCENTS
  ...
  void _calculate() {
    ...
    // Volume of square bar
    final double volume = sideM * sideM * lengthM * pieces;

    weightkg = volume * density;

    // ✅ Displayed weight according to selected unit
    double displayedWeight = convertWeight(weightkg!, selectedWeightUnit);

    // Update weighttons only if unit is ton
    weighttons = selectedWeightUnit == "Ton"
        ? displayedWeight
        : weightkg! / 1000;

    // Cost calculation always based on kg
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            _suffixInputWithDropdown(
              hintText: "Enter Steel Cost", // Label
              ...
              onChanged: (v) {
                ...
                _calculate(); // ✅ Call calculation after changing weight unit
              },
            ),
