...
class ChannelBarCalculator extends ConsumerStatefulWidget {
  const ChannelBarCalculator({super.key});

  @override
  ConsumerState<ChannelBarCalculator> createState() =>
      _ChannelBarCalculatorState();
}

class _ChannelBarCalculatorState extends ConsumerState<ChannelBarCalculator> {
  // THEME ACCENTS
  ...
  void _calculate() {
    ...
    final double baseWeightKg = volume * density;

    setState(() {
      weightkg = convertWeight(
        baseWeightKg,
        selectedWeightUnit,
      ); // convert to selected unit
      weighttons = baseWeightKg / 1000; // metric tons (reference)
      cost = baseWeightKg * costPerKg; // cost always per kg
    });
    _saveHistory();
  }
          ...
          children: [
            Center(
              child: SvgPicture.asset(
                'assets/icons/channel_bar_icon.svg',
                height: 85,
                color: orangeColor,
              ),
            ),
            ...
            _suffixInputWithDropdown(
              hintText: "Enter Length Value", // Label
            ...
            _suffixInputWithDropdown(
              hintText: "Enter Steal Cost Value",
