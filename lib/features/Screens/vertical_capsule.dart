...
class VerticalCapsule extends ConsumerStatefulWidget {
  const VerticalCapsule({super.key});

  @override
  ConsumerState<VerticalCapsule> createState() => _VerticalCapsuleState();
}

class _VerticalCapsuleState extends ConsumerState<VerticalCapsule> {
  ...
  void _calculate() {
    ...
    final double? cylinderHeightInput = double.tryParse(base2Controller.text);
    final double? filledInput = double.tryParse(heightController.text);

    if (diameterInput == null || cylinderHeightInput == null) {
      setState(() {
        totalVolume = 0;
        filledVolume = 0;
        convertedValues = {};
    ...
    final double totalM3 =
        pi * radiusM * radiusM * cylinderHeightM +
        (4.0 / 3.0) * pi * pow(radiusM, 3) * 2; // 2 hemispheres

    // Filled volume (clamp filled height if necessary)
    double filledHeightM =
        (filledInput ?? 0) * (unitToMeter[selectedUnit] ?? 1.0);
    if (filledHeightM > cylinderHeightM + 2 * radiusM) {
      filledHeightM = cylinderHeightM + 2 * radiusM;
    }

    // Simple proportion for filled volume
    double filledM3 =
        (filledHeightM / (cylinderHeightM + 2 * radiusM)) * totalM3;
  ...
  void _saveHistory() {
    ...
    final double? cylinderHeightVal = double.tryParse(base2Controller.text);
    final double? filledVal = double.tryParse(heightController.text);

    if (diameterVal == null || cylinderHeightVal == null || filledVal == null)
      return;

    final item = VerticalCapsuleItem(
    ...
    ref.read(unifiedHistoryProvider.notifier).addVerticalCapsule(item);
  }
          ...
          children: [
            Center(
              child: SvgPicture.asset(
                'assets/icons/vertical_capsule_icon.svg',
                height: 100,
                color: orangeColor,
              ),
            ),
