...
class FrequencyUnitConverter extends ConsumerStatefulWidget {
  const FrequencyUnitConverter({super.key});

  @override
  ConsumerState<FrequencyUnitConverter> createState() =>
      _FrequencyUnitConverterState();
}

class _FrequencyUnitConverterState
    extends ConsumerState<FrequencyUnitConverter> {
  final TextEditingController frequencyController = TextEditingController();

  final Color orangeColor = const Color(0xFFFF9C00);

  String selectedUnit = "Hertz";
  ...
  void _calculate() {
    final double? val = double.tryParse(frequencyController.text);
    if (val == null || val <= 0) {
  ...
  void _saveHistory() {
    final double? inputVal = double.tryParse(frequencyController.text);
    if (inputVal == null || convertedValues.isEmpty) return;

    final item = FrequencyHistoryItem(
      inputValue: inputVal,
    ...
    ref.read(unifiedHistoryProvider.notifier).addFrequency(item);
  }
        ...
        child: Column(
          ...
          children: [
            ...
            _suffixInputWithDropdown(
              ...
              dropdownOptions: frequencyUnits,
